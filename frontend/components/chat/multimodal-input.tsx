...
// Mirrors the ALLOWED_TYPES list in the upload route so drop-validation
// and the file-input accept attribute always stay in sync.
...
function PureMultimodalInput({
  ...
  const handleDroppedFiles = useCallback(
    async (files: File[]) => {
      ...
      if (rejected > 0) {
        toast.error(
          `${rejected} file${rejected === 1 ? "" : "s"} not supported. Attach images or PDFs only.`
        );
      }
      if (allowed.length === 0) {
        return;
      ...
      setUploadQueue(allowed.map((f) => f.name));
      try {
        const uploads = await Promise.all(allowed.map((f) => uploadFile(f)));
        setAttachments((curr) => [
          ...curr,
          ...uploads.filter((a) => a !== undefined),
        ]);
      } catch {
  ...
  return (
    ...
    // biome-ignore lint/a11y/useSemanticElements: no semantic element maps to a file drop zone
    <div
      aria-label="Message composer. Drag and drop images or PDFs to attach."
          ...
          <PromptInputFooter className="px-3 pb-3">
            <PromptInputTools>
              <AttachmentsButton
                ...
                hasVision={selectedModelHasVision}
                status={status}
              />
...
function PureAttachmentsButton({
  ...
  const tooltipText = hasVision
    ? "Attach an image or PDF"
    : "Attach a PDF (this model doesn't read images)";
